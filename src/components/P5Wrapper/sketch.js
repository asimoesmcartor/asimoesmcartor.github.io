...
export default function(p) {
  ...
  class Plant {
    ...
    render(cfg) {
      ...
      const deltaGammaNoise =
        (this.play ? (p.noise(this.noiseOffset / 150 + 4000) - 0.5) * 2 : 0) *
        gamma;

      const sizeNoise =
        1 -
        p.noise(this.id + 1 * 10, this.branchesCounter * 10) **
          (randomization + 2);
      const scaledSize = size * sizeNoise;
      const dir = pg
        .createVector(0, -1) // v(0, -1) -> grow to top
        .setMag(scaledSize)
        .rotate(gamma + deltaGammaNoise);
    ...
    renderBranch(dir, depth, cfg) {
      ...
      const sizeNoise =
        1 -
        p.noise(this.id + 1 * 10, this.branchesCounter * 10) ** randomization;
      const scaledSize = size * sizeNoise;
      ...
      pg.stroke(
        hue,
        saturation,
        ((recursionDepth - depth) / recursionDepth) * 100 * brightness,
        depth / recursionDepth + 0.33
      );
      ...
      if (depth > 0 && scaledSize >= branchMinLength) {
        const alphaNoise =
          (this.play ? (p.noise(this.noiseOffset / 110 + 1000) - 0.5) * 2 : 0) *
          alpha;
        const deltaAlphaNoise =
          (this.play ? (p.noise(this.noiseOffset / 60 + 2000) - 0.5) * 2 : 0) *
          deltaAlpha;
        const deltaBetaNoise =
          (this.play ? (p.noise(this.noiseOffset / 60 + 3000) - 0.5) * 2 : 0) *
          deltaBeta;

        pg.push();
        pg.translate(rescaledDir.x, rescaledDir.y);
        const newDirA = rescaledDir
          .copy()
          .setMag(scaledSize * decCoeffA)
        ...
        pg.push();
        pg.translate(rescaledDir.x, rescaledDir.y);
        const newDirB = rescaledDir
          .copy()
          .setMag(scaledSize * decCoeffB)
  ...
  p.draw = function() {
    ...
    if (!plants.length && !$.isEmptyObject(props)) {
      ...
      props.plants.forEach((_, index) =>
        plants.push(new Plant(500, 300, !(index % 1), index))
      );
